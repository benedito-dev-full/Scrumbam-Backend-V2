...
## PADRÕES APRENDIDOS — FOLDERS MVP (DEntidade + DVincula, pós-F5)

- **DVincula N:1 = uniqueness via service, não schema**: para cardinalidade N:1 (1 project, 1 folder ativo), a unicidade é enforced em service (`updateMany(excluido=true)` + `create` em transação). Prisma não suporta unique parcial multi-coluna. Verificar que o `moveProject` usa `$transaction` com update-then-create, não create-only.
- **Idempotência de script de backfill one-shot**: um N+1 interno (findFirst por project dentro de loop no backfill) é aceitável em scripts one-shot (`prisma/scripts/`). Não conta como violação. Verificar apenas que o script NÃO é importado em runtime (grep na `src/`).
- **Comentário JSDoc de contagem no seed é débito recorrente**: o comentário da constante exportada `classes` (`"45 fixas + N especificas = Total"`) precisa ser atualizado manualmente a cada nova DClasse. Validar com ts-node que COUNTS runtime bate com o header do arquivo. O header (linha ~11) e o COUNTS runtime são as fontes de verdade.
- **Rota `unassigned` antes de `/:folderId` em NestJS**: NestJS resolve por ordem de declaração. Rotas literais específicas (`/folders/unassigned`) DEVEM vir antes de rotas com wildcard (`:folderId`) no mesmo controller, ou `unassigned` será capturado como folderId. Verificar sempre a ordem de decorators `@Get`.
- **DRY via private duplicate em vez de import cross-module**: quando service precisa de helper de outro módulo mas isso criaria circular dependency via `forwardRef`, duplicar ~30 linhas simples é aceitável como débito pragmático. Documentar via JSDoc no método privado (`@see FoldersService.resolveFolderIdsForProjects` + nota de DRY). Sinalizar para cleanup futuro em service compartilhado (ex: `common/`).
- **`Array.isArray(links)` guard defensivo em Prisma findMany**: Prisma sempre retorna array; o guard é redundante mas inofensivo. Aparece quando Implementer atualizou mocks legados que retornavam `undefined` para chamadas extras de `findMany`. Verificar se o mock foi atualizado junto (contagem correta de chamadas no spec). Registrar como M3 (cosmético, pode ser removido).
