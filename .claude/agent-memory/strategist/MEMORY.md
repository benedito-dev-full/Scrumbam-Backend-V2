...
## ADR-V2-054 — DBookmark (tabela de domínio Scrumban)

Decidido em plan-bookmarks-modulo-completo-task1.md (2026-05-26):
- DVincula NÃO é usada para bookmarks: idEntidade tem FK hard para DEntidade e DProject usa sequence independente. Adicionar colunas a DVincula criaria débito arquitetural no template framework.
- Nova tabela `DBookmark` com FK polimórfica sem constraint hard (validada no service). Unique nativa `(userEntidadeId, targetType, targetId)`.
- ADR-V2-001 se aplica às 17 canônicas do template; DBookmark é domínio Scrumban — tabela autorizada via ADR-V2-054.
- ATENÇÃO: verificar hook `enforce-canonical-tables.sh` antes da migration (pode precisar de allowlist).
- BUG LATENTE em projects.service.ts:895 — `dvVincula.idEntidade = projectId` (FK para DEntidade mas recebe ID de DProject). Não é escopo desta task. Task futura: plan-projects-dvincla-fk-refactor-task2.md.
...
| Bookmarks-Task1 | `workspace/plans/plan-bookmarks-modulo-completo-task1.md` | 2026-05-26 | DBookmark tabela própria (não DVincula). canAccessProject() adicionado em ProjectsService (método público, reusa lógica Camada A+B de findMany). Batch polimórfico em GET: 1q bookmarks + 1q projects (Promise.all por tipo). DELETE /bookmarks/:id (não body). Soft-delete. AuthCompositeGuard. Sem paginação no MVP. BUG LATENTE DVincula.idEntidade=projectId em projects.service.ts:895 — TODO task separada. |
