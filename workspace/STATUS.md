...
## 🎯 Plano Órfão (F4 Pós-F3) — Estado órfão: usuário sem workspace ativa — ✅ COMPLETA (5/5 etapas)

Plano `workspace/plans/plan-orphan-workspace.md`. Ciclo completo Strategist → Implementer → Reviewer → Documenter.

### ✅ Plano Completo (5 etapas integradas)

**Implementer completou:** 2026-05-14 (etapas 1-5 sobre 5 dias)
**Reviewer aprovou:** 2026-05-14 (score médio 8.6/10 across 5 etapas: 8.8|8.2|8.5|8.7|8.8)
**Documenter finaliza:** 2026-05-14 (formalização ADR-V2-038, CHANGELOG, STATUS, commit)
**Módulo V2:** `src/auth/` (decorators, guards, strategy, service, DTOs)
**Build:** `make build` PASS
**Tests:** 114/114 PASS (auth + invites + organizations)

**Etapas implementadas (score → status):**

| # | Tema | Score | Status |
|---|------|-------|--------|
| 1 | JWT `organizationId` opcional + remover fallback | 8.8/10 | ✅ APPROVED |
| 2 | `@AllowOrphan()` decorator + `RequireWorkspaceGuard` + relaxar guards | 8.2/10 | ✅ APPROVED |
| 3 | Destravar login órfão + `isOrphan: boolean` + DEvento -501 + `switchOrg` migrado | 8.5/10 | ✅ APPROVED |
| 4 | `GET /auth/pending-invites` + `InvitesService.listPendingInvitesForEmail()` + DTO | 8.7/10 | ✅ APPROVED |
| 5 | `@AllowOrphan()` em `POST /organizations` + smoke E2E + ADR esboço | 8.8/10 | ✅ APPROVED |

**Média geral:** (8.8 + 8.2 + 8.5 + 8.7 + 8.8) / 5 = **8.6/10 APPROVED**
...
**Guards (novos + modificados):**
- ✅ `src/auth/guards/require-workspace.guard.ts` — Bloqueia JWT órfão com 403 `{ code: 'NO_WORKSPACE' }` em rotas sem `@AllowOrphan()`
- ✅ `src/auth/guards/auth-composite.guard.ts` — Injeta `RequireWorkspaceGuard` após autenticação Passport
- ✅ `src/auth/guards/org-tenant.guard.ts` — Relaxado: retorna `true` para órfão (deixa `RequireWorkspaceGuard` cuidar)
- ✅ `src/auth/guards/roles.guard.ts` — Relaxado: idem (idem)

**Strategy:**
- ✅ `src/auth/strategies/jwt.strategy.ts` — `validate()` aceita payload sem `organizationId`

**AuthService:**
- ✅ `src/auth/auth.service.ts` — `login()`, `refresh()`, `issueSessionForUser()` emitem JWT órfão; `getMe()` retorna `isOrphan: boolean`; DEvento -501 com `metaDados.orphan: true`

**DTOs:**
- ✅ `src/auth/dto/auth-response.dto.ts` — `UserProfileDto.isOrphan: boolean` (obrigatório)
- ✅ `src/auth/dto/pending-invite-for-me.dto.ts` — 5 campos, zero leak (inviteId, orgId, orgName, role, expiresAt)

**Endpoints com `@AllowOrphan()`:**
- ✅ `GET /auth/me` — retorna `isOrphan: true` para usuário sem workspace
- ✅ `POST /auth/logout` — usuário órfão pode sair
- ✅ `POST /auth/switch-org` — caminho de saída do estado órfão
- ✅ `GET /auth/pending-invites` — novo, lista convites pendentes (sanitizado, zero leak)
- ✅ `POST /organizations` — criar primeira workspace (novo decorator aplicado)

**Services:**
- ✅ `src/invites/invites.service.ts` — `listPendingInvitesForEmail(email)`: 2 queries (findMany PENDING, batch IN para resolução de nomes)

**Testes (114/114 PASS):**
- ✅ `src/auth/guards/require-workspace.guard.spec.ts` — 5 cenários (rota pública, @AllowOrphan, JWT órfão, normal, sem req.user)
- ✅ `src/auth/auth.service.spec.ts` — 8 novos (login órfão, refresh órfão, issueSessionForUser órfão, isOrphan flag, regressão zero)
- ✅ `src/invites/invites.service.spec.ts` — 8 (listPendingInvitesForEmail com/sem, batch resolve, time-based filters, sanitização)
- ✅ `src/invites/invites.controller.spec.ts` — 4 (200, 403 not-orphan-allowed, etc.)

**Smoke E2E:**
- ✅ `workspace/smoke/smoke-orphan-workspace-etapa5.md` — 2 fluxos manuais (A: criar org, B: aceitar convite)

**ADR:**
- ✅ `docs/decisions/ADR-V2-038-orphan-jwt.md` (renumerado de 040 — resolvido conflito de numeração com HMAC bilateral) — Status PROPOSED, Documenter formalizou, exemplos consolidados

### Pilares

- **Pilar 1 (Engine):** N/A — Auth é cadastro estrutural (Prisma direto em transaction)
- **Pilar 2 (Endpoints):** ✅ REUTILIZADO — `@AllowOrphan()` aplicado em controllers existentes (AuthController, OrganizationsController); zero novo controller
- **Pilar 3 (Seed):** ✅ RESPEITADO — ZERO DClasses novas (reutiliza -46 USER_GROUP, -150 USER, -501 LOGIN_EVENT, -152 ORG, -180 DEFAULT_TEAM, -475 ISSUE_COUNTER, -161/-181 vínculos)

### Fluxo de Saída do Estado Órfão

1. **Criar workspace:** `POST /organizations` → DEntidade -152 + equipe padrão → `POST /auth/switch-org` → JWT novo com `organizationId`
2. **Aceitar convite:** `GET /auth/pending-invites` → `POST /invites/:token/accept` → JWT novo com `organizationId`

### Regressão Zero

- ✅ Users com org ativa: fluxos login/refresh/token idênticos
- ✅ Smoke E2E Task #2 (convites) continua PASS
- ✅ Build + 114 tests PASS
...
| JSDoc | 100% métodos públicos |
| N+1 Queries | ZERO (batch, 2 queries em `listPendingInvitesForEmail`) |
| Regressão | ZERO (fluxos antigos continuam) |

### Agents Performance

| Agent | Duration | Quality |
|-------|----------|---------|
| Strategist | — | Plan 5 etapas integradas |
| Implementer | ~12h (5 dias) | 5 etapas APPROVED (8.8, 8.2, 8.5, 8.7, 8.8) |
| Reviewer | ~2h | Score médio 8.6/10 (5 reviews) |
| Documenter | ~1h | JSDoc ✅, ADR formalizado ✅, CHANGELOG ✅, STATUS ✅, commit Conventional ✅ |

### Referências

- **Plan:** `workspace/plans/plan-orphan-workspace.md`
- **Reviews:** `workspace/reviews/review-*-orphan*.md` (5 arquivos, scores 8.8/8.2/8.5/8.7/8.8)
- **Implementation notes:** `.claude/agent-memory/implementer/orphan-*.md` (5 arquivos, detalhes por etapa)
- **Smoke:** `workspace/smoke/smoke-orphan-workspace-etapa5.md` (2 fluxos manuais)
- **ADR:** ADR-V2-038 (JWT órfão é estado válido), ADR-V2-001/-003/-008/-028 (referências cruzadas)

**Status:** ✅ COMPLETO — Pronto para go-live. Frontend pode usar `isOrphan` + 403 `NO_WORKSPACE` para renderizar empty state.
